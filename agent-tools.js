...
async function searchOperations(input) {
  ...
  return {
    ...
    payables: (data.payables || []).filter(item => match(item, `${villaName(item.villaId)} bill bills payable payables vendor supplier tagihan utang`)).slice(0, limit).map(item => ({ ...compactRecord(item, ['id', 'code', 'vendorName', 'villaId', 'category', 'description', 'amount', 'currency', 'issueDate', 'dueDate', 'status', 'paidDate', 'paymentMethod', 'proofUrl', 'notes', 'sourceId']), villaName: villaName(item.villaId) })),
    tasks: data.villaTasks.filter(item => match(item, `${villaName(item.villaId)} task tasks maintenance todo pekerjaan tugas`)).slice(0, limit).map(item => ({ ...compactRecord(item, ['id', 'title', 'villaId', 'category', 'priority', 'status', 'dueDate', 'assignee', 'cost', 'notes']), villaName: villaName(item.villaId) })),
  };
}
