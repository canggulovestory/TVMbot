...
const BOT_NUMBER    = '+6287777278270';
...
function passesGroupRules(jid, text) {
  ...
  // silent mode → never respond
  if (mode === 'silent') return false;

  // always mode → respond to everything
  if (mode === 'always') return true;

  // mention_only mode → require @bot keyword
  const lc = text.toLowerCase();
  const botKeywords = (cfg.settings?.botMentionKeywords || ['@bot', '@tvmbot', 'agent']);

  if (botKeywords.some(kw => lc.includes(kw.toLowerCase()))) return true;

  // triggerKeywords also trigger a response in mention_only
  if (group.triggerKeywords && group.triggerKeywords.length > 0) {
    if (group.triggerKeywords.some(kw => lc.includes(kw.toLowerCase()))) return true;
  }

  return false;
}

// ── Get group config for a JID ────────────────────────────────────────────────
function getGroupConfig(jid) {
  const cfg = loadConfig();
  return (cfg.groups || []).find(g => g.jid === jid) || null;
}

// ── Check if a sender is a group admin (Baileys) ──────────────────────────────
...
// ── Send ──────────────────────────────────────────────────────────────────────
// Outbound gate: when NO_AUTONOMOUS_MODE is on, only allow sends explicitly
// authorized by the operator through the chat flow.
// Authorized paths set global.__tvmbot_authorized_send = true around their call.
function _isAuthorizedOutbound() {
  if (!global.__tvmbot_no_autonomous) return true;
  // Operator-confirmed direct send via /chat
  if (global.__tvmbot_authorized_send) return true;
  // Replies to incoming WA messages we directly received are still allowed —
  // those are tagged with global.__tvmbot_replying_to_inbound by whatsapp.js.
  if (global.__tvmbot_replying_to_inbound) return true;
  return false;
}

async function sendMessage(jid, text) {
  if (!isConnected || !sock) throw new Error('WhatsApp not connected');
  if (!_isAuthorizedOutbound()) {
    log(`🚫 BLOCKED outbound to ${jid} (NO_AUTONOMOUS_MODE — no operator authorization)`);
    throw new Error('BLOCKED_BY_NO_AUTONOMOUS_MODE');
  }
  ...
  try {
    ...
    sock.ev.on('connection.update', async (update) => {
      ...
      if (connection === 'close') {
        ...
        qrCodeBase64    = null;   // clear expired QR on close
        emitter.emit('state', connectionState);
      ...
      for (const msg of messages) {
        ...
        if (m && (m.audioMessage || m.pttMessage) && _voiceHandler) {
          ...
          try {
            ...
            if (reply) {
              global.__tvmbot_replying_to_inbound = true;
              try { await sendMessage(replyJid, reply); }
              finally { global.__tvmbot_replying_to_inbound = false; }
            }
          } catch(e) {
        ...
        if (!isGroup) {
          ...
          if (_messageHandler) {
            try {
              ...
              if (reply) {
                global.__tvmbot_replying_to_inbound = true;
                try { await sendMessage(senderJid, reply); }
                finally { global.__tvmbot_replying_to_inbound = false; }
              }
        ...
        // Detect native WhatsApp @mention (mentionedJid contains bot's number)
        const mentionedJids = msg.message?.extendedTextMessage?.contextInfo?.mentionedJid || [];
        const botNum = BOT_NUMBER.replace('+', ''); // '6287777278270'
        const isBotNativeMention = mentionedJids.some(j => j.startsWith(botNum));
        if (!passesGroupRules(groupJid, text) && !isBotNativeMention) {
        ...
        if (_messageHandler) {
          try {
            ...
            if (reply) {
              global.__tvmbot_replying_to_inbound = true;
              try { await sendMessage(groupJid, reply); }
              finally { global.__tvmbot_replying_to_inbound = false; }
            }
...
module.exports = {
  ...
  getStatus, getGroups,
  updateGroupConfig, loadConfig, getGroupConfig, isGroupAdmin,
  setMessageHandler,
