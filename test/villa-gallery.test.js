...
test('villa gallery keeps the chosen order and rejects non-web URLs', async () => {
  ...
  const villa = await villaData.upsert('villas', { name: 'Villa LYSA', gallery, yearlyRate: 300000000, paymentTerms: '1 year upfront', publicWhatsApp: '6282112111511' });
  assert.deepEqual(villa.gallery, gallery.slice(0, 2));
  assert.equal(villa.yearlyRate, 300000000);
  assert.equal(villa.paymentTerms, '1 year upfront');
  assert.equal(villa.publicWhatsApp, '6282112111511');
