...
// Pub/Sub config
const PUBSUB_TOPIC = 'projects/tvmbot-gmail/topics/gmail-notifications';
...
class EmailWatcher {
  ...
  async startWatch(topicName) {
    try {
      ...
      console.error('[EmailWatcher] Watch start error:', err.message);
      throw err;
    }
  }
  ...
  async _processEmail(messageId) {
    try {
      ...
      if (classification.type === 'booking_platform') this.state.stats.airbnb++;
      if (classification.type === 'bank_payment') this.state.stats.bank++;
      this._saveState();
