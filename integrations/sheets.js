...
async function writeSheet(spreadsheetId, range, values) {
  try {
    const res = await getClient().spreadsheets.values.update({
      spreadsheetId, range,
      valueInputOption: 'USER_ENTERED',
      requestBody: { values: Array.isArray(values[0]) ? values : [values] }
    });
    const d = res.data || {};
    console.log(`[Sheets] Write OK: ${d.updatedRows || 0} rows → ${d.updatedRange || range}`);
    return { success: true, updatedRange: d.updatedRange, updatedRows: d.updatedRows, updatedColumns: d.updatedColumns };
  } catch (err) {
...
async function appendSheet(spreadsheetId, sheetName, rowValues) {
  try {
    ...
    console.log(`[Sheets] Append OK: ${updates.updatedRows || 0} rows → ${updates.updatedRange || range}`);
    return {
      success: true,
      updatedRange: updates.updatedRange || null,
      updatedRows: updates.updatedRows || row.length,
    ...
    return { success: false, error: err.message, spreadsheetId, sheetName };
  }
}
