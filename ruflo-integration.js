...
function processMessage(message, sender, context) {
  ...
  try {
    ...
    if (driftDetector) {
      try {
        var health = driftDetector.getStats();
...
function runMaintenance() {
  ...
  if (driftDetector) {
    try {
      results.drift = { health: driftDetector.getStats(), checkpoint: driftDetector.createCheckpoint() };
    } catch (e) {}
  }
...
function getDashboard(timeRange) {
  ...
  try {
    ...
    if (driftDetector) dashboard.drift = driftDetector.getStats();
    return dashboard;
